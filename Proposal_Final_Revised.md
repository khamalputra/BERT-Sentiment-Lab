...
1. ***McNemar’s Test***
 Digunakan pada level prediksi sampel individu pada *Held-out Test Set* (872 sampel) untuk menguji apakah perbedaan proporsi kesalahan klasifikasi bersifat simetris atau asimetris (McNemar, 1947; Dror et al., 2018). Matriks kontingensi $2 \times 2$ dibangun secara khusus dari pasangan prediksi sampel pada *seed* acuan tunggal yang seragam (*Seed 42*, *seed* pertama dalam sekuens *multi-seed*). Pemilihan *seed* tunggal yang identik untuk kedua model ini menjamin asas *ceteris paribus* (mencegah distorsi akibat variasi stokastik inisialisasi yang berbeda antar-model). Uji ini menghasilkan statistik $\chi^2$ dan nilai $p$-value.
2. ***Wilcoxon Signed-Rank Test***
