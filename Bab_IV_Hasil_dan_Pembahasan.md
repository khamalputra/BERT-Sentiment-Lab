...
2. **Fallback Backend (CPU Server)**: Berjalan di Railway Cloud Service (1-vCPU Server) sebagai server cadangan otomatis jika runtime GPU Colab dalam keadaan non-aktif (*offline*), melayani inferensi dengan latensi rerata $\sim 24{,}88\text{ ms}$.
...
Ditinjau dari dinamika pelatihan (*Epoch Stop*), sesuai dengan spesifikasi *hyperparameter* pada **Tabel 3.4 Proposal** (di mana batas maksimum pelatihan ditetapkan sebesar 10 epoch untuk Model A dan 5 epoch untuk Model B dengan *patience early stopping* = 3 epoch), pada Model A variasi titik henti pelatihan antara epoch 7 hingga 10 mengindikasikan bahwa konvergensi tidak selalu tercapai secara konsisten sebelum batas maksimum. Khususnya pada seed 777 dan 2024, pelatihan menyentuh batas maksimum 10 epoch tanpa terpicunya mekanisme *early stopping*, karena *Validation F1-score* terus berfluktuasi secara terbatas. Sebaliknya, Model B secara konsisten menyentuh batas optimum dan memicu penghentian pelatihan tepat pada epoch ke-5 di seluruh 6 *random seed*, mengindikasikan bahwa proses *fine-tuning end-to-end* memiliki kurva konvergensi yang jauh lebih stabil, cepat, dan efisien.
...
4. **Kapasitas Throughput Server Cadangan (CPU Fallback)**: Arsitektur *Dual-Backend Hybrid* terbukti sukses menjamin *High Availability* aplikasi web dengan pengalihan failover otomatis ($1{,}24\text{ detik}$). Meskipun latensi inferensi sampel tunggal pada CPU Fallback Railway Cloud tercatat sangat responsif ($\sim 24{,}88\text{ ms} < 100\text{ ms}$), *throughput* pemrosesan batch data berukuran besar pada CPU tetap memiliki keterbatasan kapasitas komputasi dibandingkan akselerasi GPU NVIDIA A100 Primary.
