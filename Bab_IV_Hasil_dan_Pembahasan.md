...
2. **Kompromi Sumber Daya Komputasi dan Analisis Temuan Latensi/VRAM Inferensi (Sintesis RQ3 & RQ4)**: Meskipun Model B mengungguli Model A secara signifikan dalam metrik prediktif F1-Score ($92{,}94\%$ vs $86{,}66\%$), hasil pengujian komputasi menunjukkan temuan menarik di mana profil latensi inferensi GPU A100 ($1{,}82\text{ ms}$ vs $1{,}83\text{ ms}$) dan konsumsi VRAM puncak teralokasi ($3177{,}18\text{ MB}$ vs $3173{,}13\text{ MB}$) bernilai hampir persis identik dengan selisih VRAM hanya $-4{,}05\text{ MB}$ ($<0{,}15\%$). Temuan ini dijelaskan secara arsitektural oleh fakta bahwa pada saat inferensi (*forward pass / model.eval()*), kedua model mengeksekusi struktur *backbone* `bert-base-uncased` yang persis sama dengan 110 juta parameter ($O(N)$ FLOPS perkalian matriks yang identik). Perbedaan *fine-tuning* vs *feature extraction* pada Model B hanyalah pada penyesuaian nilai bobot *internal attention*, bukan pada struktur arsitektur atau jumlah parameter aktif. Perbedaan konsumsi memori VRAM yang besar sebenarnya hanya terjadi selama fase *pelatihan* (*training pass*), di mana Model B memerlukan *gradient state storage* dan *optimizer AdamW state* (2x parameter) untuk seluruh 110M parameter, sedangkan Model A hanya menyimpan *gradient state* untuk *linear classification head*. Pada tahap inferensi tanpa *gradient tracking* (`with torch.no_grad()`), konsumsi VRAM murni ditentukan oleh ukuran bobot model statis + *PyTorch CUDA Caching Allocator* (yang mencadangkan memori konstan $3584\text{ MB}$ untuk kedua model). Sementara pada lingkungan CPU Fallback Server (Railway Cloud 1-vCPU), latensi inferensi rerata tercatat sebesar $24{,}65\text{ ms}$ (Model A) dan $25{,}12\text{ ms}$ (Model B) (rerata gabungan $\sim 24{,}88\text{ ms}$). Hasil perbandingan ini mengonfirmasi bahwa penyiapan server cadangan berbasis CPU tetap mampu melayani inferensi pengguna secara responsif ($< 100\text{ ms}$) ketika server GPU utama mengalami kendala konektivitas.
