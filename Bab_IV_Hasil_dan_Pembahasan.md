...
Untuk menjawab potensi ketidaksesuaian intuitif di mana Model B melatih 110 juta parameter namun menghasilkan alokasi VRAM yang identik dengan Model A pada Tabel 4.2c, dilakukan pembedaan analisis antara **Fase Pelatihan (Training Phase)** dan **Fase Inferensi Production (Inference Phase)**:

1. **Fase Pelatihan (Training Phase)**: Selama fase pelatihan (*backpropagation autograd* & pembaruan bobot AdamW), Model B membutuhkan memori VRAM puncak sebesar **$\sim 8{,}20\text{ GB}$** ($8.396\text{ MB}$), karena PyTorch harus mengalokasikan memori GPU untuk *gradient tensor storage* serta *AdamW optimizer 1st & 2nd moment states* ($m_t, v_t$) untuk seluruh 110 juta parameter. Sebaliknya, Model A (*Feature Extraction*) hanya membutuhkan VRAM pelatihan sebesar **$\sim 3{,}80\text{ GB}$** ($3.891\text{ MB}$), karena *autograd gradient* dan *optimizer state* hanya dialokasikan khusus untuk 1.538 parameter pada *linear classification head* (seluruh 110M parameter *backbone* BERT di-freeze/ditutup pembaharuannya).
2. **Fase Inferensi Production (Inference Phase)**: Setelah proses pelatihan selesai dan model diposisikan dalam mode evaluasi tanpa *gradient tracking* (`model.eval()`, `with torch.no_grad()`), seluruh *gradient buffer* dan *optimizer state* secara otomatis dibebaskan (*deallocated*) dari VRAM GPU. Pada tahap inferensi production yang dilaporkan pada Tabel 4.2b dan 4.2c, kedua model mengeksekusi struktur matriks 110M parameter `bert-base-uncased` yang persis sama, sehingga alokasi memori inferensi puncak (*Peak Allocated VRAM*) bernilai konstan dan identik pada **$\sim 3173$--$3177\text{ MB}$** ($\sim 3{,}17\text{ GB}$) dengan memori tercadang (*Peak Reserved VRAM*) konstan pada **$3584{,}00\text{ MB}$** ($3{,}50\text{ GB}$).
